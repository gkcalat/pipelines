...
type APIRun struct {
	...
	RuntimeDetails *APIRuntimeDetails `json:"runtime_details,omitempty"`
...
func (m *APIRun) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateRuntimeDetails(formats); err != nil {
		res = append(res, err)
	}
...
func (m *APIRun) validateRuntimeDetails(formats strfmt.Registry) error {

	if swag.IsZero(m.RuntimeDetails) { // not required
		return nil
	}

	if m.RuntimeDetails != nil {
		if err := m.RuntimeDetails.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("runtime_details")
			}
			return err
		}
	}
